...
describe('UndirectedGraph', () => {
  ...
  it('dijkstra should handle disconnected components', () => {
    graph.addEdge('A', 'B', 1);
    graph.addEdge('C', 'D', 1);
  ...
  it('dijkstra should handle multiple paths to the same node', () => {
    graph.addEdge('A', 'B', 5);
    graph.addEdge('A', 'C', 2);
    graph.addEdge('C', 'B', 1);
    ...
    expect(distances.get('B')).toBe(3); // A → C → B is shorter
  });

  it('dijkstra should handle zero-weight edges', () => {
    graph.addEdge('A', 'B', 0);
    graph.addEdge('B', 'C', 1);
  ...
  it('dijkstra should throw if start vertex is not in graph', () => {
    expect(() => graph.dijkstra('X')).toThrow(
      'Start vertex not found in the graph.',
    );
  });

  it('dijkstra should handle single-vertex graph', () => {
    graph.addVertex('A');
